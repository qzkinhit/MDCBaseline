...
import numpy as np
import copy
from Cleaner.Holistic_BigDansing.mvc import read_graph_dc
from Cleaner.Holistic_BigDansing.tes import greedy_min_vertex_cover_dc
    ...
    def run(self, file_path, dirty_path, clean_path):
            ...
            if (constantflag == 1):
                if (i_othernum == 0 and i_equalnum == 0):
                    ...
                    if (i_ocjoinnum == 0):
                        ...
                        for idx in range(len(self.sorts)):
                            self.sorts[idx].clear()
                icnt += 1
                self.maypair.append([])
        ...
        if not self.PERFECTED:
            ...
            out_path = os.path.join(self.output_path, "Exp_result", self.task_name[:-1],
                                    "onlyED_" + self.task_name + check_string(self.dirty_path.split("/")[-1]) + ".txt")
            # out_path = "../../results/bigdansing/Exp_result/" + self.task_name[:-1] +"/onlyED_" + self.task_name + check_string(dirty_path.split("/")[-1]) + ".txt"
            os.makedirs(os.path.dirname(out_path), exist_ok=True)  # 创建目录
            f = open(out_path, 'w')
            sys.stdout = f
            ...
            os.makedirs(os.path.dirname(out_path), exist_ok=True)  # 创建目录
            os.makedirs(os.path.dirname(res_path), exist_ok=True)  # 创建目录
